...
# ggplot stacks in layers via '+' sign
ggplot(myTPM.stats) + 
  aes(x = SD, y = MED) +
  geom_hex(shape=4, size=3) +
  theme_cowplot()
...
p1 <- ggplot(log2.cpm.df.pivot) +
  aes(x=samples, y=expression, fill=samples) + # fill = color
...
keepers <- rowSums(cpm>1)>=5 # keep genes with cpm > 1 in at least 5 samples
